...
- The visionOS viewer uses a compact floating control window for server search, explicit connection, optional automatic immersive entry, default hidden-window immersive mode with return-to-menu re-entry/disconnect controls, optional keep-window-visible behavior, visible-hands upper-limb control, and head pose, hand joints, and first-pass tracked accessory controller data back to the runtime when available.
